...
func (s *stmt) start(args []driver.Value) error {
	...
	for i, v := range args {
		switch v := v.(type) {
		case int8:
			if rv := C.duckdb_bind_int8(*s.stmt, C.ulong(i+1), C.schar(v)); rv == C.DuckDBError {
				return errCouldNotBind
			}
			continue
		case int16:
			if rv := C.duckdb_bind_int16(*s.stmt, C.ulong(i+1), C.short(v)); rv == C.DuckDBError {
				return errCouldNotBind
			}
			continue
		case int32:
			if rv := C.duckdb_bind_int32(*s.stmt, C.ulong(i+1), C.int(v)); rv == C.DuckDBError {
				return errCouldNotBind
			}
			continue
		case int64:
			...
			continue
		case float64:
			if rv := C.duckdb_bind_double(*s.stmt, C.ulong(i+1), C.double(v)); rv == C.DuckDBError {
				return errCouldNotBind
			}
			continue
		case bool:
			if rv := C.duckdb_bind_boolean(*s.stmt, C.ulong(i+1), true); rv == C.DuckDBError {
				return errCouldNotBind
			}
			continue
		case time.Time:
		...
		case string:
			str := C.CString(v)
			if rv := C.duckdb_bind_varchar(*s.stmt, C.ulong(i+1), str); rv == C.DuckDBError {
